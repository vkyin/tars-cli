...
module.exports = async function (ctx, { tempDir }, {
  ...
  ctx.print(`准备发布 "${appName}.${serverName}" on "tars-dev"`)
  ctx.print('如果有问题，赶紧Ctrl C')
  await wait(1500)

  ctx.print('正在拉取服务信息')
  const { data: serverlist, ret_code: serverListReqCode } = await request({ method: 'GET', url: ctx.conf.tarswebUrl + '/api/tree', json: true })
  ...
  let d = await request({
    method: 'GET',
    url: ctx.conf.tarswebUrl + '/api/server_list',
    json: true,
    qs: {
      'tree_node_id': app.id
    }
  ...
  let uploadPatchPackageResult = await request({
    method: 'POST',
    url: ctx.conf.tarswebUrl + '/api/upload_patch_package',
  ...
  const task_no = await createTask(serverListIds, 'patch_tars', {
    patch_id: packageInfo.id, // 版本号
    update_text: '发布自脚本', // 备注
    bak_flag: true // 备机标识，true：备机，false：主机
  })
  ctx.print('发布任务创建成功. taskNo', task_no)

  ctx.print('正在轮询任务状态')
  await intervalTask(task_no, 2)
  ctx.print('发布成功')

  ctx.print('正在添加重启任务')
  const restart_no = await createTask(serverListIds, 'restart')
  ctx.print('重启任务添加成功。')
  await intervalTask(restart_no, 2)
  ctx.print('done')

  async function createTask (serverListIds, command, param) {
    const task = await request({
      method: 'post',
      url: ctx.conf.tarswebUrl + '/api/add_task',
      headers: {
  ...
  async function getTaskInfo (taskNo) {
    const task = await request({
      method: 'GET',
      url: ctx.conf.tarswebUrl + '/api/task',
      headers: {
        'content-type': 'application/json'
      },
      json: true,
