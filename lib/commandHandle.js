...
module.exports = {
  ...
  async TarsPatch (ctx, { tempDir }, {
    ...
    const cmd = `${path.resolve(__dirname, './../tars-web.js')} patch ${appName} ${serverName} -u ${ctx.conf.tarswebUrl}`
  ...
  async TarsDeploy (ctx, { tempDir }, {
    ...
    const tarsDeployCmdPath = path.resolve(__dirname, './../node_modules/.bin/tars-deploy')
    const { exec } = require('child_process')
    await new Promise((resolve, reject) => {
